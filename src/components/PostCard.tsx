...
export default function PostCard({ post, onDelete, currentUser }: PostCardProps) {
  ...
  const handleArchiveDeletion = async (e: React.MouseEvent) => {
    ...
    const confirmed = window.confirm('Are you sure you want to incinerate this post? This cannot be undone.');
    if (!confirmed) return;
    ...
    try {
      console.log(`[UI] Optimistic Incineration started: ${post.id}`);
      ...
      if (res.ok) {
        console.log('[UI] Incineration successful');
        onDelete?.(post.id);
      } else {
        // ROLLBACK: Show again if it failed
        setIsHidden(false);
        setIsDeleting(false);
        console.error('[UI] Incineration failed:', result.error || 'Unknown error');
        alert(`Incineration failed: ${result.error || 'Server Error'}`);
      }
    } catch (err) {
      // ROLLBACK: Show again on network error
      setIsHidden(false);
      setIsDeleting(false);
      console.error('[UI] Critical error during deletion:', err);
