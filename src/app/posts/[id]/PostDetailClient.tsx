...
export default function PostDetailClient({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-[#131313] text-[#F0ECE3] pb-32 md:pb-12">
      {/* Header with Safe Area Nav Fix */}
      <header className="sticky top-0 z-30 bg-[#131313]/90 backdrop-blur-xl border-b border-white/5 px-4 pt-[max(env(safe-area-inset-top),1rem)] pb-2 h-auto min-h-[4rem] flex items-center justify-between">
      ...
      <main className="max-w-2xl mx-auto">
        ...
        <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
          <button onClick={() => inputRef.current?.focus()} className="flex items-center gap-2 text-[#6B6B6B] hover:text-[#1d9bf0] transition-colors group">
          ...
          <button onClick={handleLike} className={`flex items-center gap-2 transition-colors group ${isLiked ? 'text-[#ff535b]' : 'text-[#6B6B6B] hover:text-[#ff535b]'}`}>
          ...
          <button onClick={handleBookmark} className={`flex items-center gap-2 transition-colors group ${isBookmarked ? 'text-[#fbbf24]' : 'text-[#6B6B6B] hover:text-[#fbbf24]'}`}>
            <div className="p-2 rounded-full group-hover:bg-[#fbbf24]/10 transition-colors">
              <Bookmark size={20} fill={isBookmarked ? 'currentColor' : 'none'} />
            </div>
          </button>

          <button onClick={handleShare} className="flex items-center gap-2 text-[#6B6B6B] hover:text-green-400 transition-colors group">
            <div className="p-2 rounded-full group-hover:bg-green-400/10 transition-colors">
              <Share2 size={20} />
            </div>
            <AnimatePresence mode="wait">
              {copied && <motion.span key="copied" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="font-inter text-xs text-green-400">Copied!</motion.span>}
            </AnimatePresence>
          </button>
        </div>
          ...
          <div className="px-4 py-3 border-b border-white/5 flex items-start gap-3">
            ...
            <div className="flex-1 flex flex-col gap-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleCommentSubmit} disabled={!commentText.trim() || submitting} className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#ff535b] text-white font-syne font-bold text-xs uppercase tracking-wider disabled:opacity-30 disabled:grayscale hover:bg-[#ff6b72] transition-all active:scale-95">
                  {submitting ? <div className="w-3.5 h-3.5 border border-white/30 border-t-white rounded-full animate-spin" /> : <Send size={13} />}
                  Reply
                </button>
              </div>
            </div>
        ...
        <div className="flex flex-col">
          {replies.length === 0 ? (
            ...
            replies.map((reply, idx) => (
              <motion.div key={reply.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }} className="px-4 py-4 border-b border-white/5 flex gap-3 hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="font-syne font-bold text-[13px] text-[#F0ECE3]">{reply.user?.display_name || reply.user?.username || 'Anonymous'}</span>
                    <span className="font-inter text-[11px] text-[#4A4A4A]">@{reply.user?.username || 'anon'}</span>
